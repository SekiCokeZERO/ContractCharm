...
contract Factory {
    ...
    function getBytecode() external view { //0x52c7420d
        assembly {
            let usingTSTORE := tload(_usingTSTORE.slot)

            let slotContent
            switch usingTSTORE
            case 0 {
                slotContent := sload(_bytecode.slot)
            }
            default {
                slotContent := tload(_bytecode.slot)
            }
            ...
            case 1 { //length >= 32 bytes
                ...
                default {
                    for { let i } lt(i, iter) { i := add(i, 0x01) } {
                        mstore(add(0x40, mul(i, 0x20)), tload(add(_bytecodeExpandSlot, i)))
                    }
                }
...
contract Loader {
    ...
    constructor() {
        assembly {
            mstore(0x00, 0x52c7420d00000000000000000000000000000000000000000000000000000000) //getBytecodeS()
